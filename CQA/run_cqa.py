...
def check_pred(predictions ,dev_dataset, fp):
    ...
    for line_ in tqdm(predictions, ncols=100, desc="check questions where model failed to answer"):
        ...
        for i in range(len(ls)):
            if ls[i]["qid"] == qid_:
                s = ls[i]
                data.append({"qid":qid_, "question":s["question"], "answers":s["answers"], "label":s["label"], "id":id_})
                #sample_ = "{}, {}, {}, {}, {}{}".format(qid_ ,s["question"], s["answers"], s["label"], id_, os.linesep)
                #fp.write(sample_)
   # data.to_csv(fp, mode="w")
   # fp.close()
    torch.save(data, fp)
...
def main():
    ...
    parser.add_argument("--check_predict", default=1, type=int,help="Eval sample question and predicted answers")
...
#  CUDA_VISIBLE_DEVICES=3 python run_cqa.py --data_dir ./a_path_to/rand
#  --do_train --do_eval --max_seq_length 80 --do_lower_case --adam_epsilon 1e-6
#  --max_grad_norm 1. --weight_decay 0.01 --warmup_proportion 0.1 --learning_rate 5e-5
#  --max_steps 3400 --eval_steps 500 --train_batch_size 12 --eval_batch_size 6
#  --gradient_accumulation_steps 1 --model_class bert --model_name_or_path bert_pretrain
#  --output_dir glm_bert_CQA --seed 42 --check_predict 1 --overwrite_output_dir